...
contract SimpleBank {
    ...
    function deposit() public payable returns (uint) {
      ...
      require(enrolled[msg.sender], 'Sender Should be enrolled !') ;
      require(msg.value >= 0, 'Value is not valid') ;
      // 3. Add the amount to the user's balance. Hint: the amount can be
      //    accessed from of the global variable `msg`
      uint256 finalBalance = balances[msg.sender] + msg.value ;
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
      ...
      uint256 firstBalance = balances[msg.sender] ;
      // 1. Use a require expression to guard/ensure sender has enough funds
      require(firstBalance >= withdrawAmount, 'Sender has not enough balance !') ;
      // 2. Transfer Eth to the sender and decrement the withdrawal amount from
      //    sender's balance
      uint256 finalBalance = firstBalance - withdrawAmount ;
      balances[msg.sender] = finalBalance ;
      msg.sender.transfer(withdrawAmount) ;
      // 3. Emit the appropriate event for this message
      emit LogWithdrawal(msg.sender, withdrawAmount, finalBalance) ;
    }
}
